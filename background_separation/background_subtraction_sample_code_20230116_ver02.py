...
cv2.destroyAllWindows()
#%% 04. Backgroud Subtraction_total_전체 영상에 대해서 모두 다루는 경우 !
...
#%% 04. Backgroud Subtraction_total_Modify_전체영상이 1분이 넘어가는 경우 !
...
count_value = 0
for idx, i in tqdm(enumerate(range(length))):
    return_value, frame = vid.read()
    ...
    if return_value:
        ...
        ### method 01 / cv2로 시각화 - temp (바로 보임)
        # cv2.imshow('background extraction video', visualization_img) # PRESS 'SPACE' to progress rain streak visualizaion & 
        # cv2.waitKey()
        
        
        ### method 02 / plt로 시각화 - temp (plot에 결과가 남으나, 메모리 소요가 큼)
        # plt.figure()
        # plt.axis('off')
        # plt.imshow(rain_streak, cmap='gray')
        
        
        ### method 03 / cv2로 폴더에 직접 저장
        cv2.imwrite(r'./#rain_streak/REF_'+digit_four(idx)+'.png', rain_streak)
                
    if idx == theshold_second*fps:
        # print(idx)
        # cv2.destroyAllWindows()
        break
